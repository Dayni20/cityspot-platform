...
function ActivityCard({ activity, favorite = false, onFavorite }) {
  ...
  return <article className="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
    ...
    <div className="p-5"><div className="flex items-center gap-1 text-sm text-slate-500"><MapPin size={15}/>{activity.city}</div><h3 className="mt-2 text-lg font-bold text-slate-950">{activity.name}</h3><p className="mt-2 line-clamp-2 text-sm leading-6 text-slate-600">{activity.description}</p><div className="mt-4 flex flex-wrap gap-3 text-xs text-slate-500"><span className="flex items-center gap-1"><DollarSign size={14}/>{price === "N/D" ? price : `$${price}`}</span><span className="flex items-center gap-1"><Clock size={14}/>{schedule.split(" ").slice(0,3).join(" ")}</span></div><Link to={`/activities/${activity.id}`} className="mt-5 inline-flex w-full justify-center rounded-xl bg-brand-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-brand-700">Ver detalle</Link></div>
  </article>;
}
