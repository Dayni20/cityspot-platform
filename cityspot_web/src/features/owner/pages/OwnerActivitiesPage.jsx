  ...
  return (
    <section>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <Link to="/owner/activities/new" className="flex items-center justify-center gap-2 rounded-xl bg-brand-600 px-4 py-3 font-bold text-white">
          <PlusCircle size={18} />
          Nueva
        </Link>
      </div>
            ...
            <article key={activity.id} className="grid gap-4 rounded-2xl border bg-white p-4 shadow-sm md:grid-cols-[1fr_auto]">
              <div>
                ...
                <p className="mt-2 text-sm text-slate-500">
                  {activity.city} - Categoria {activity.categoryId} - ${activity.referencePrice ?? "N/D"}
                </p>
              ...
              <div className="flex items-center gap-2 md:flex-col md:justify-center">
                <Link to={`/owner/activities/${activity.id}/edit`} className="flex items-center gap-2 rounded-xl border px-3 py-2 text-sm font-semibold">
                  <Edit size={16} />
                  Editar
                </Link>
                <Link to={`/owner/activities/${activity.id}/images`} className="flex items-center gap-2 rounded-xl border px-3 py-2 text-sm font-semibold">
                  <Image size={16} />
                  Imagenes
                </Link>
                <button onClick={() => deactivate(activity.id)} className="flex items-center gap-2 rounded-xl border border-red-200 px-3 py-2 text-sm font-semibold text-red-600">
                  <Trash2 size={16} />
                  Eliminar
                </button>
              </div>
