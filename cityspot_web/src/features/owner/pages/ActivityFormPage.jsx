  ...
  return (
    <section>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <Link to="/owner/activities" className="flex items-center gap-2 text-sm font-semibold text-slate-600">
            <ArrowLeft size={17} />
            Volver
          </Link>
      ...
      <form onSubmit={submit} className="mt-8 grid gap-5 rounded-2xl border bg-white p-6 shadow-sm sm:grid-cols-2">
        ...
        <Field label="Categoria">
          <select required name="categoryId" value={form.categoryId} onChange={change} className="field">
        ...
        <Field label="Descripcion" wide>
          <textarea required name="description" value={form.description} onChange={change} className="field min-h-28" />
        </Field>
        <Field label="Ciudad">
          <input required name="city" value={form.city} onChange={change} className="field" />
        </Field>
        <Field label="Direccion">
          <input name="address" value={form.address} onChange={change} className="field" />
        </Field>
        <Field label="Latitud">
          <input type="number" step="any" name="latitude" value={form.latitude} onChange={change} className="field" />
        </Field>
        <Field label="Longitud">
          <input type="number" step="any" name="longitude" value={form.longitude} onChange={change} className="field" />
        </Field>
        <Field label="Precio referencial">
          <input type="number" min="0" name="referencePrice" value={form.referencePrice} onChange={change} className="field" />
        </Field>
        <Field label="Horario">
          <input name="schedule" value={form.schedule} onChange={change} className="field" />
        </Field>
        <Field label="Telefono">
          <input name="contactPhone" value={form.contactPhone} onChange={change} className="field" />
        </Field>
        <Field label="Correo">
          <input type="email" name="contactEmail" value={form.contactEmail} onChange={change} className="field" />
        </Field>
        <button disabled={loading} className="flex items-center justify-center gap-2 rounded-xl bg-brand-600 py-3 font-bold text-white disabled:opacity-60 sm:col-span-2">
          <Save size={18} />
